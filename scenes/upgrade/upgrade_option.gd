...
func setup(upgrade: UpgradeData) -> void:
	...
	name_label.position = Vector2(text_inset, text_top)
	name_label.size = Vector2(text_width, 32)
	name_label.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_banner.add_child(name_label)

	var desc_y := text_top + 36.0
	var desc_label := Label.new()
	desc_label.text = upgrade.description
	desc_label.add_theme_font_size_override("font_size", 22)
	desc_label.autowrap_mode = TextServer.AUTOWRAP_WORD
	desc_label.position = Vector2(text_inset, desc_y)
	desc_label.size = Vector2(text_width, banner_size.y - desc_y - 20.0)
...
func set_selected(selected: bool) -> void:
	...
	if _is_selected:
		...
		_bracket.wrap_rect(content, 32.0, 32.0)
	else:
		if _bracket:
			_bracket.queue_free()
			_bracket = null
